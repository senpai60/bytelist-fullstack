...
const Badge = ({ children, color = "bg-gray-800 text-gray-100", className = "" }) => (
  <span className={`inline-flex items-center rounded px-2 py-1 text-xs font-semibold border ${color} ${className}`}>
    {children}
  </span>
);
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="text-xl font-bold mb-2 text-white">{task.title}</div>
        ...
        {task.progress.length > 0 && (
          <div className="space-y-2 mt-4">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium text-zinc-200">{task.progress.length} steps completed</span>
            </div>
            <Progress value={progressPercentage} className="h-2 w-40 bg-zinc-400" />
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-zinc-700 mt-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-zinc-200">{task.duration} min</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="pt-4 border-t border-zinc-700 mt-4">
            ...
            <div className="flex flex-wrap gap-1">
              {task.sources.slice(0, 3).map((source, idx) => (
                <Badge key={idx} color="bg-zinc-700 text-white border-zinc-800" className="text-xs">
                  {source}
                </Badge>
              ))}
              {task.sources.length > 3 && (
                <Badge color="bg-zinc-700 text-white border-zinc-800" className="text-xs">
